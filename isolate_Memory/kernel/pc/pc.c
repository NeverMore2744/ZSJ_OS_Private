...
setup_kstack(task_struct *proc) {
	...
	int dummy_stack_start = (unsigned int)(dummy_setup_kstack_area);
	int dummy_stack_end   = (unsigned int)(dummy_setup_kstack_area + 65536);//4096*16 = 65536 //MAX_PID
	...
	if(dummy_setup_kstack_count<MAX_PID-1)
	{
        ...
        kernel_printf("[setup_kstack] (minus 0x80000000)Stack buffer area from :%d, to :%d, allocating addr: %d\n",
                                             dummy_stack_start - 0x80000000,
                                             dummy_stack_end  - 0x80000000,
