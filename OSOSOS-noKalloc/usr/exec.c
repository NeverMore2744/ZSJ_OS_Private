...
static char dummy_memory_remove_kmalloc_for_exec[4096];
int exec(char* filename) {
    ...
    unsigned int i = 0;
    unsigned int j = 0;
    unsigned int ENTRY = (unsigned int)dummy_memory_remove_kmalloc_for_exec;//(unsigned int)kmalloc(4096);
    for (j = 0; j < n; j++) {
...
#endif  // ! EXEC_DEBUG
    int r = f();
    //kfree((void*)ENTRY);//match kmalloc
    return r;
}
