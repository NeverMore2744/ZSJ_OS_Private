...
void *kmalloc(unsigned int size) {
    ...
    if (size > kmalloc_caches[PAGE_SHIFT - 1].objsize) {
        size += (1 << PAGE_SHIFT) - 1;
        size &= ~((1 << PAGE_SHIFT) - 1);
        return (void *)(KERNEL_ENTRY | (unsigned int)alloc_pages(size >> PAGE_SHIFT)); // use buddy_alloc
    }
...
void kfree(void *obj) {
    ...
    if (!(page->flag == _PAGE_SLAB))
        return free_pages((void *)((unsigned int)obj & ~((1 << PAGE_SHIFT) - 1)), page->bplevel); // use buddy_free
