...
int sd_read_sector_blocking(int id, void* buffer) {
    ...
    code = sd_send_cmd_blocking(0x1139, id);   //发送命令0x1139
    if (code != 0)  //检查命令状态
        goto ret;
    do {
        code = SD_CTRL[15];  //等待数据传输
    } while (code == 0);
    if (code & 1) {                    //检查数据状态
        for (i = 0; i < 128; i++)      //从缓冲区复制数据
...
int sd_write_sector_blocking(int id, void* buffer) {
    ...
    // Fill buffer
    for (i = 0; i < 128; i++)
        SD_BUF[i] = buffer_int[i]; //向缓冲区复制数据
    code = sd_send_cmd_blocking(0x1859, id); //发送命令0x1859
    if (code != 0)  //检查命令状态
        goto ret;
    do {
        code = SD_CTRL[15]; //等待数据传输
