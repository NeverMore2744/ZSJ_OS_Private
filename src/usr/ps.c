...
void parse_cmd() {
    ...
    //new instruction  "trySem" --philosopher problem using semaphore
    else if(kernel_strcmp(ps_buffer, "trySem") == 0)
    {
        kernel_puts("Try: philosopher problem using semaphore, note that this function would not return to shell.\n", 0x00f, 0);
        {
    ...
    else if(kernel_strcmp(ps_buffer, "tryTrueSem") == 0)
    {            
        extern void true_sem_check_sync(void);//syncronization checking version 2
         // check philosopher sync problem 
         true_sem_check_sync();//this function never return. kernel::syncronization::truesemtest.c
    }    

    //new instruction  "trySyscall"
	else if(kernel_strcmp(ps_buffer, "trySyscall") == 0)
	{
		char try_syscall_4_szString[] = "[PearlShell] try_syscall_4_szString\n";
	...
	else if (kernel_strcmp(ps_buffer, "showimg") == 0) {
		result = showimg(param);
		kernel_printf("showimg return with %d\n", result);
	}
    else if (kernel_strcmp(ps_buffer, "pgtabletest") == 0) {
        test_pgtable();
        kernel_printf("pgtabletest return with 0\n");
    }
    else if (kernel_strcmp(ps_buffer, "bmpPicturesTest") == 0) {
        test_g();
        kernel_printf("bmpPicturesTest return with 0\n");
    }
    else if (kernel_strcmp(ps_buffer, "MemBigTest") == 0) {
        test_slab_and_buddy();
    }
    else if (kernel_strcmp(ps_buffer, "TextMode") == 0) {
        cvt2Tmode();
    }
    else if (kernel_strcmp(ps_buffer, "GraphicsMode") == 0) {
        cvt2Gmode();
    }
    else if (kernel_strcmp(ps_buffer, "clearGraphics") == 0) {
        clear_graphics();
    }
    else if (kernel_strcmp(ps_buffer, "clearAll") == 0) {
        clear_black();
    }
	else if (kernel_strcmp(ps_buffer, "writetest") == 0) {
		result = writetest();
		kernel_printf("writetest return with %d\n", result);
	} 
	else{
