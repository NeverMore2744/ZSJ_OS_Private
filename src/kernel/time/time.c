...
void system_time_proc() {
    ...
    while (1) {
        ...
        for (i = 0; i < 16; i++)
            kernel_putchar_at(day[i], 0xfff, 0, 29, 56 + i);
        for (i = 0; i < 8; i++)
...
void get_time(char *buf, int len) {
    assert(len >= 9, "Buf of get_time too small, at least 9 bytes");  // Ensure that the buffer is more than 8 bytes
    unsigned int ticks_high, ticks_low;
    asm volatile(
        "mfc0 %0, $9, 6\n\t"
        "mfc0 %1, $9, 7\n\t"
