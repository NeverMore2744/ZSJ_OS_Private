...
struct lock_t kernel_printf_lock;
...
// bg: background color, 12 bits
// fc: fore-character color, 12 bits
// Print the string at a certain location
...
int kernel_printf(const char *format, ...) {
    lockup(&(kernel_printf_lock));
    int cnt = 0;
    va_list ap;
    va_start(ap, format);
    cnt = kernel_vprintf(format, ap);
    va_end(ap);
    unlock(&(kernel_printf_lock));
    return cnt;
}
