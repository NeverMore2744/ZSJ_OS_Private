...
/* get*/
u32 get_entry_start_cluster(u8 *entry) {
	return (u32)((get_u16(entry + 20) << 16) + (get_u16(entry + 26)));
}
...
u32 clus2sec(u32 clus, struct fs_info* fatinfo)
{
	return ((clus - 2) << fs_wa(fatinfo->BPB.attr.sectors_per_cluster)) + fatinfo->first_data_sector;
}
