...
PAGES PAGE_BUFFER;   // È«¾Öpages»º´æ
...
/* Initialize the buffer, according to the fat_info */
...
u32 fat_fs_write_4k(BUF_4K *f, struct fs_info * fat_info) {
	if ((f->cur != 0xffffffff) && (f->cur != 0x00000000)/* Important!!! or may damage the sd card*/ && (((f->state) & 0x02) != 0)) {
		if (write_block(f->buf, f->cur, fat_info->BPB.attr.sectors_per_cluster) == 1)
			goto fs_write_4k_err;
