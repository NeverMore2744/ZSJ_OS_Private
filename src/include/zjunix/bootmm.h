...
struct bootmm {
    unsigned int phymm;    // the actual physical memory, in total in the machine
    unsigned int max_pfn;  // record the max page number
    unsigned char* s_map;  // bit-map begin place, a memory address
    unsigned char* e_map;  // bit-map end place, a memory address
    unsigned int last_alloc_end;  // The last allocation address
    unsigned int cnt_infos;  // get number of infos stored in bootmm now
    struct bootmm_info info[MAX_INFO]; // cnt_infos < MAX_INFO
};
